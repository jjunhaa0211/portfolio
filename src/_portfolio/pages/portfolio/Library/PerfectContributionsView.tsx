...
const PerfectContributionsView = () => {
  ...
  이 과정에서 수학적 지식이 필요하여 고등학교 수학 선생님에게 조언을 얻어 삼각형, 다각형과 같은 복잡한 도형의 경우에는 각도와 변의 길이를 계산할 수 있었습니다.
  
  ##### 도형의 개수를 어떻게 받아야 할까?
  \`문제 상황\`
  사용자로부터 데이터를 받는 방법을 고민했을 때, 처음에는 단일 값만을 받아서 가로나 세로가 항상 정사각형이 되는 문제가 생겼어요.
  \`해결 방법\`
  ...
  처음으로 라이브러리를 **CocoaPods, SPM, 카르타고를 통해 배포**해 보며 다양한 플랫폼을 배포해보는 경험을 가지게 되었습니다. 다양한 플랫폼을 배포하여 여러 사용자가 저의 라이브러리를 사용할 수 있도록 하였으며 라이브러리 공개 후 몇몇 사용자와 소통하며 큰 뿌듯함을 느꼈고, 특히 한 사용자가 저의 라이브러리로 첫 앱을 만들었다는 사실에 기쁨을 느꼈습니다.
  ...
  return (
    <>
      <Top>
        ...
        <Txt typography={"H0"}>
          PerfectContributionsView <Txt typography={"P1"}>2022.06.01~</Txt>
        </Txt>
      </Top>
