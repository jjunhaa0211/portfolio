...
const Maeumgajim = () => {
  ...
  \`해결 방법\`
  💡모듈로직을 Domain과 Network를 Data 의존성을 부여하는 그래프로 변경하여서 프로젝트 내에서 Domain인 Network를 가지거나 Domain이 Data를 알게 되면 에러가 나오도록 처리하였습니다. 그리고 코드 내에서 **프로토콜을 사용**하여 통신하는 구조로 바꾸어서 모든 모듈에는 전 단계의 프로토콜을 만들어줌으로써 **ViewModel과 UseCase간의 통신을 원활**하게 하였으며 이 밖에서 프로토콜을 사용하여 Repository, service로직을 **서로 다른 모듈에서 접근**할 수 있도록 구현했습니다.
  ...
  \`해결 방법\`
  Snapshot Test를 도입하면서 팀원이 원하는 코드를 테스트를 실행하여서 스크린샷 파일을 **빌드 파일보다 훨씬 빨리 가져올 수 있게 되었습니다.** 또한 VoiceOver를 도입하면서 QA 시간에 정해진 UI를 테스트할 수 있도록 뱅크샐러드에서 만든 AXSnapshot 수정해서 만든 **Mango라는 마음가짐 전용 테스트** 라이브러리를 개발하게 되었습니다. 이로인해서 iOS 개발자들은 Snapshot Test를 도입하면서 **소통 시간을 단축**하였고 Mango를 사용하여 프로젝트의 **QA 시간을 단축**할 수 있습니다.`)
  ...
  const { Result: Result5 } = useMoonerDown(`모듈화를 통한 **빌드 속도 (19.5초 -> 8.2초) 단축**`)
  ...
  return (
    <>
      ...
      <Tags>
        {["Swift", "Snpkit", "Then", "RxSwift", "RxFlow", "Moya", "OAuth", "swiftSnapshotTesting", "Tuist", "MVVM"].map((v) => DefaultTag[v as keyOfDefaultTag])}
      </Tags>

      <URLBox>
      <URLItem
          icon={"마음가짐"}
          text={"운동 숏폼 시청, 루틴, 운동 방법 등을 제공하는 종합 헬스 애플리케이션"}
        >
          <Txt>한줄 요약</Txt>
        </URLItem>
      </URLBox>
      ...
      <P>
        ...
        감소 문제를 해결하는 것이 마음가짐의 목적입니다. 6월초 1차 배포를
        앞두고 있습니다.
      </P>
      ...
      <P>• iOS 개발자</P>
      <P2>
        MVVM을 사용하여서 inout 개념을 도입했습니다.
      </P2>
      <P2>
        MVVM을 보다 쉽게 사용하기 위해서 제네릭을 통한 View와 ViewModel 연결 방식을 채택했습니다.
      </P2>
      <P2>
        클린 아키텍처를 도입하고 Data, Domain, Network를 사용해 서버 연동을 구현했습니다.
      </P2>
      <P2>
      ...
      <Details>
        ...
        <div style={{ display: "flex", paddingBottom: 30 }}>
          ...
          <Img
            ...
            />
            {/* <div style={{ display: "flex"}}> */}
          {/* <video src={Video1} width={205} controls></video>
          </div>
          <div style={{ display: "flex", paddingLeft: 50}}>
          <video src={Video2} width={205} controls></video>
          </div> */}
          </div>
        </div>
      </Details>
