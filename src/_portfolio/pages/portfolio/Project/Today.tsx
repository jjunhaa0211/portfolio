...
### 프로젝트 주요 기능
##### 인생네컷 기능
- 사진 4개를 갤러리에서 가져와 지정된 프레임 사진 추가
- 인생네컷 전용 프레임을 추가 기능
##### 폴라로이드 기능
- 갤러리에서 사진을 한 장 가져와 지정된 프레임 사진 추가
- 폴라로이드 전용 프레임 추가
- 수정한 폴라로이드 사진앱 저장
##### 스티커 기능
- 스티커를 지정하여 인생네컷 또는 폴라로이드에서 스티커를 추가 및 조정 가능
- 스티커의 크기, 모양 조절 및 삭제
##### 카메라 기능
- 사용자가 희망하는 포즈를 제공하여 포즈에 맞게 사진 촬영
- 화면 전환 가능
- 화면 확대 및 축소
- 카메라가 아래를 향할 시 경고
- 화면을 터치해서 사진 촬영
...
\`해결 방법\`
  ...
  그리고 저희는 이벤트 부스인 만큼 사용자와 기업에게 즉석 피드백을 받으며 단시간 내에 기능을 추가하는 작업 등을 진행했습니다. 이 과정에서 사용자들이 원하는 기능과 니즈의 중요성을 깨닫게 되었습니다.  
  이번 경험을 통해, 단순히 기능 구현에 초점을 맞추는 것에서 벗어나, 코드의 질을 향상시키고 적합한 아키텍처 적용의 중요성을 깊이 이해하게 되었습니다.
...
사용자가 특정 세그먼트를 클릭했을 때, 해당 세그먼트에 맞는 위치로 즉각적으로 움직여야한다는 문제가 발생했습니다.
  ...
  return (
    <>
      ...
      <Tags>{["Swift", "Snpkit", "Then", "XCameraKit"].map((v) => DefaultTag[v as keyOfDefaultTag])}</Tags>

      <URLBox>
      <URLItem
          icon={"Today"}
          text={"손 쉽게 만드는 폴라로이드 인생 네컷 애플리케이션"}
        >
          <Txt>한줄 요약</Txt>
      </URLItem>
      </URLBox>
