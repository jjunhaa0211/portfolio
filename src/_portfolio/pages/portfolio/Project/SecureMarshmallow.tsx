...
const SecureMarshmallow = () => {
  ...
  const { Result: Result2 } =
    ...
    \`해결 방법\`
    첫번째 시도
    - UserDefaults를 암호화 해서 사용자에게 복호화 방법을 진행하려고 했습니다. 하지만 정해진 복호화 양식은 사용자가 코드를 보고 유추할 수 있다는 의견이 보안팀에서 나왔습니다.
    두번째 시도
    - keychain을 사용해서 애플이 제공해주는 암호화 방법을 선택했습니다.
    - keychain을 사용하면 모든 항목이 암호화 되어 이슈를 해결할 수 있습니다.
    추가적인 방법을 도입
    - 저희 팀에서는 탈옥된 기기를 막자는 의견이 나왔고 서칭을 해본 결과 IOSSecuritySuite를 사용해서 탈옥 감지 모듈을 찾아낸 후 탈옥 모듈일 경우 앱에 접근도 하지 못하게 nil을 반환해서 강제로 종료하도록 개발했습니다.
    ...
    이뿐만 아니라 직접 **버전이 낮은 아이폰을 구매하여 직접 탈옥**을 시도하였고 **Frida를 사용**하여 앱의 접근해보았습니다. 소스코드를 뜯어보는 작업과 강제로 앱을 종료하는 작업을 구현했습니다.
