...
### 역할
Tuist 모듈을 최적화해서 RIBs 아키텍처에 맞도록 설계하는 작업을 했습니다. (아래 토글을 참고해주세요)
...
앱의 빌드 시간과 메모리 최적화를 위해 staticLibrary와 dynamicLibrary로 모듈을 분할하는 과정에서 Tuist는 성공적으로 작동했으나, Xcode에서 의존성 문제로 인해 앱 실행이 실패하는 문제가 발생했습니다.`)
  ...
  이러한 과정에서 여러 어려움을 겪었지만, 동시에 중요한 교훈을 얻을 수 있었습니다.
  ...
  const {Result: Result4} = useMoonerDown(`Tuist의 의존성 문제를 해결하기 위해 **Xcode Build Setting**을 점검한 결과, **bundleId의 중복이 문제의 원인**임을 발견했습니다. 각 모듈은 고유한 아이디를 가져야 하는데, 중복된 bundleId로 인해 DemoApp을 찾을 수 없는 상황이었습니다.
  ...
  ###### 핵심 아키텍처 모듈
  UserInterface 모듈과 함께, 모든 UserInterface 모듈에는 클린 아키텍처를 위해 만들어진 **Data와 Domain 모듈이 필수적으로 포함**됩니다. 이러한 모듈들은 \`invertedBualTargetProject\`로 만들어지며, DemoApp과 같은 별도의 데모 테스트가 필요하지 않습니다. 하지만 사용자가 유닛 테스트 또는 UI 테스트가 필요하다면 적용할 수 있습니다. 또한, Network나 TokenManager와 같은 다른 모듈들도 마찬가지로 \`invertedBualTargetProject\`로 구현됩니다.
