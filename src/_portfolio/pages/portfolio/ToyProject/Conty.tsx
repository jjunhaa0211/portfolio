import {
  ...
  const Conty = () => {
    ...
    const { Result } = useMoonerDown(`### 개요
      ...
      ### 회고
      하루 동안 만든 프로젝트로서 기능 구현 및 UI 제작까지는 8시간이 걸렸습니다. 하지만 애플 심사 때문에 이틀을 소모하게 되었습니다.
      이번 프로젝트를 하면서 UIPasteboard를 사용하여 데이터를 복사하는 방법을 공부할 수 있었습니다. GoogleMobileAds를 사용하여 앱 하단의 배너를 추가하는 방법에 대해서 배우고 이를 통한 수익 창출까지 도전해보았습니다.
      추후 개발적인 측면에서 추가할 기능은 복사되었을 때 바로 공유하기 버튼을 하단에 놓아서 복사한 이모티콘을 바로 내보낼 수 있는 기능과 즐겨찾기 기능을 추가할 예정이고 유지 보수적인 측면에서는 애플 워치, 맥북 등등을 지원할 계획이며 현재 MVC인 프로젝트를 TCA로 바꾸며 RxSwift를 전면 도입할 생각입니다. 현재 앱스토어에서 *콘티(Conty)*를 만나볼 수 있습니다.
      `);
      return (
        <>
          <Top>
