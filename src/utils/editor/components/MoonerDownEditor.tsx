...
const MoonerDownEditor = ({ texts, setTexts, renders, placeholder }: Props) => {
			...
			case 'Enter':
				...
				if (checks) {
					...
					if (!s) {
						...
						edit.setSelectionRange(start + 1 + tab.length, end + 1 + tab.length)
						start += 1 + tab.length
						end += 1 + tab.length
						updateText(edit)
						registerUndo()
	...
	return (
		<>
			...
			{/*<MainDiv onClick={event => editor.current?.focus()} contentEditable suppressContentEditableWarning>*/}
			{/*    {blocks.compileEditor(texts)*/}
			{/*        .map((block, index) => <EditDiv onClick={() => {*/}
			...
			<MDTextArea
				...
				onMouseDown={(event: { button: number; preventDefault: () => void; currentTarget: HTMLTextAreaElement }) => {
				...
				onMouseUp={(event: any) => {
					...
					// if (edit && selection && selection.rangeCount > 0) {
					//     const range = document.createRange();
					//     range.setStart(edit, edit.selectionStart)
					//     range.setEnd(edit, edit.selectionEnd)
